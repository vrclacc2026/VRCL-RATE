...
import { calcFormula, applyExtraCost, roundPackingValue } from './rate-calculator.js?v=20260915-udaan-editable-v2';
...
async function ensureReference(target,source,cloudValue,targetRates){
  ...
  const firstEditableMigration=existing.udaanEditorVersion!==2;
  const state={...existing,packingRateReference:{city:'Ahmedabad',productId:source.id},packingRateReferenceLocked:firstEditableMigration?false:existing.packingRateReferenceLocked===true,udaanEditorVersion:2,rows};
  delete state.looseReference;delete state.looseReferenceLocked;
  meta[key]=state;
  const changed=firstEditableMigration||existing.packingRateReference?.city!=='Ahmedabad'||existing.packingRateReference?.productId!==source.id||!!existing.looseReference||Object.prototype.hasOwnProperty.call(existing,'looseReferenceLocked');
...
function installModeStyle(){
  ...
  const style=document.createElement('style');style.id='vrclUdaanModeStyle';
  style.textContent='.vrcl-udaan-mode .masterRow,.vrcl-udaan-mode #looseRefRow{display:none!important}.vrcl-udaan-mode #packingRefRow{display:flex!important}';
  document.head.appendChild(style);
}
function applyUdaanUi(){
  installModeStyle();
  const udaan=selectedCity()==='Udaan';
  document.body?.classList.toggle('vrcl-udaan-mode',udaan);
  if(!udaan)return;
  const loose=$('looseRate');if(loose){loose.disabled=true;loose.title='Not used for Udaan. Packing MASTER comes from Ahmedabad reference.'}
}
function render(){
...
async function loadUdaan(){
  applyUdaanUi();
  if(selectedCity()!=='Udaan'){editor=null;return}if(busy||!selectedId())return;busy=true;const key=selectedKey(),id=selectedId();
  try{
...
if($('rateBody')){
  ...
  document.addEventListener('click',e=>{if(e.target.closest('.productBtn')||e.target.closest('.city')){editor=null;setTimeout(applyUdaanUi,0);setTimeout(applyUdaanUi,250);schedule(650);return}if(selectedCity()!=='Udaan'||!editor)return;if(e.target.closest('#packingRefLock')){e.preventDefault();e.stopImmediatePropagation();void toggleUdaanLock();return}const del=e.target.closest('[data-udaan-delete]');if(del){e.preventDefault();e.stopImmediatePropagation();void deleteRow(+del.dataset.udaanDelete);return}if(e.target.closest('#saveAll')){e.preventDefault();e.stopImmediatePropagation();void saveUdaan();return}if(e.target.closest('#addPacking')){e.preventDefault();e.stopImmediatePropagation();toast('Udaan packing comes automatically from the matching Ahmedabad product.')}},true);
  const observer=new MutationObserver(()=>{applyUdaanUi();if(selectedCity()==='Udaan'&&editor&&!document.querySelector('#rateBody [data-udaan-row]'))schedule(60)});
  observer.observe(document.querySelector('main')||document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['class','hidden']});
  applyUdaanUi();schedule(900);setTimeout(()=>schedule(0),2000);
}